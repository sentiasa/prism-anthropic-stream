...
it('can generate text with a basic stream', function (): void {
    ...
    expect($chunks[$lastChunkIndex]->meta)->toBeInstanceOf(Meta::class);
    expect($chunks[$lastChunkIndex]->meta->id)->not()->toBeEmpty();
    expect($chunks[$lastChunkIndex]->meta->model)->not()->toBeEmpty();
...
it('adds rate limit data to the responseMeta', function (): void {
    ...
    expect($firstChunk->meta->rateLimits)->toHaveCount(4);
    expect($firstChunk->meta->rateLimits[0])->toBeInstanceOf(ProviderRateLimit::class);
    expect($firstChunk->meta->rateLimits[0]->name)->toEqual('requests');
    expect($firstChunk->meta->rateLimits[0]->limit)->toEqual(1000);
    expect($firstChunk->meta->rateLimits[0]->remaining)->toEqual(500);
    expect($firstChunk->meta->rateLimits[0]->resetsAt)->toEqual($requests_reset);
});
